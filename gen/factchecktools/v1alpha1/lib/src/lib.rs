...
mod resources {
    pub mod claims {
        ...
        impl<'a, A: yup_oauth2::GetToken> SearchRequestBuilder<'a, A> {
            ...
            pub fn iter_claims<T>(self) -> SearchClaimsIter<'a, A, T>
            where
        ...
        pub struct SearchClaimsIter<'a, A, T> {
            method: SearchRequestBuilder<'a, A>,
            last_page_reached: bool,
            items_iter: Option<::std::vec::IntoIter<T>>,
        }
        impl<'a, A, T> Iterator for SearchClaimsIter<'a, A, T>
        where
    ...
    pub mod pages {
        ...
        impl<'a, A: yup_oauth2::GetToken> ListRequestBuilder<'a, A> {
            ...
            #[doc = r" populated fields in the yielded items will be determined by the"]
            #[doc = r" `FieldSelector` implementation."]
            pub fn iter_claim_review_markup_pages<T>(
                self,
            ) -> ListClaimReviewMarkupPagesIter<'a, A, T>
            where
                T: ::serde::de::DeserializeOwned + ::field_selector::FieldSelector,
            {
                ListClaimReviewMarkupPagesIter {
                    method: self,
                    last_page_reached: false,
                    items_iter: None,
                }
        ...
        pub struct ListClaimReviewMarkupPagesIter<'a, A, T> {
            method: ListRequestBuilder<'a, A>,
            last_page_reached: bool,
            items_iter: Option<::std::vec::IntoIter<T>>,
        }
        impl<'a, A, T> Iterator for ListClaimReviewMarkupPagesIter<'a, A, T>
        where
            A: ::yup_oauth2::GetToken,
            T: ::serde::de::DeserializeOwned,
        {
