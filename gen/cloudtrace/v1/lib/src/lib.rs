...
mod resources {
    pub mod projects {
        ...
        #[derive(Debug, Clone)]
        pub struct PatchTracesRequestBuilder<'a, A> {
            pub(crate) reqwest: &'a ::reqwest::Client,
            pub(crate) auth: &'a ::std::sync::Mutex<A>,
            request: crate::schemas::Traces,
            project_id: String,
        ...
        impl<'a, A: yup_oauth2::GetToken> PatchTracesRequestBuilder<'a, A> {
        ...
        pub mod traces {
            ...
            impl<'a, A: yup_oauth2::GetToken> TracesActions<'a, A> {
            ...
            impl<'a, A: yup_oauth2::GetToken> GetRequestBuilder<'a, A> {
                ...
                pub fn execute_standard(
                    self,
                ) -> Result<crate::schemas::Trace, Box<dyn ::std::error::Error>> {
                    self.execute_fields::<_, &str>(None)
                }
                #[doc = r" Execute the given operation. This will provide a `fields`"]
                #[doc = r" selector of `*`. This will include every attribute of the"]
                ...
                fn _request(&self, path: &str) -> ::reqwest::RequestBuilder {
                    ...
                    let req = req.query(&[("$.xgafv", &self.xgafv)]);
                    let mut auth = self.auth.lock().unwrap();
                    let fut = auth.token(vec!["https://www.googleapis.com/auth/trace.readonly"]);
                    let mut runtime = ::tokio::runtime::Runtime::new().unwrap();
                    let token = runtime.block_on(fut).unwrap().access_token;
                    let req = req.bearer_auth(&token);
                    req
            ...
            impl<'a, A: yup_oauth2::GetToken> ListRequestBuilder<'a, A> {
                ...
                pub fn iter_traces<T>(self) -> ListTracesIter<'a, A, T>
                where
                    T: ::serde::de::DeserializeOwned + ::field_selector::FieldSelector,
                {
                    ListTracesIter {
                ...
                #[doc = r" the server."]
                pub fn iter_traces_standard(
                    mut self,
                ) -> ListTracesIter<'a, A, crate::schemas::Trace> {
                    self.fields = Some(concat!("nextPageToken,", "traces").to_owned());
                    ListTracesIter {
                        method: self,
                        last_page_reached: false,
                        items_iter: None,
                ...
                #[doc = r" resources."]
                pub fn iter_traces_debug(mut self) -> ListTracesIter<'a, A, crate::schemas::Trace> {
                    self.fields = Some(concat!("nextPageToken,", "traces", "(*)").to_owned());
                    ListTracesIter {
                        method: self,
                        last_page_reached: false,
                ...
                pub fn execute_standard(
                    self,
                ) -> Result<crate::schemas::ListTracesResponse, Box<dyn ::std::error::Error>>
                {
                    self.execute_fields::<_, &str>(None)
                }
                #[doc = r" Execute the given operation. This will provide a `fields`"]
            ...
            pub struct ListTracesIter<'a, A, T> {
                method: ListRequestBuilder<'a, A>,
                last_page_reached: bool,
                items_iter: Option<::std::vec::IntoIter<T>>,
            }
            impl<'a, A, T> Iterator for ListTracesIter<'a, A, T>
            where
