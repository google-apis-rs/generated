...
pub mod resources {
    pub mod applications {
        ...
        impl<'a, A: yup_oauth2::GetToken> ListRequestBuilder<'a, A> {
            ...
            pub fn iter_applications<T>(mut self) -> crate::iter::PageItemIter<Self, T>
            where
                T: ::serde::de::DeserializeOwned + ::field_selector::FieldSelector,
            {
                let mut fields = concat!("nextPageToken,", "applications").to_owned();
            ...
            #[doc = r" the server."]
            pub fn iter_applications_standard(
                mut self,
            ) -> crate::iter::PageItemIter<Self, crate::schemas::Application> {
                self.fields = Some(concat!("nextPageToken,", "applications").to_owned());
                crate::iter::PageItemIter::new(self, "applications")
            }
            #[doc = r" Return an iterator that iterates over all `#prop_ident`. The"]
            #[doc = r" items yielded by the iterator are `#items_type`. The populated"]
            ...
            #[doc = r" resources."]
            pub fn iter_applications_debug(
                mut self,
            ) -> crate::iter::PageItemIter<Self, crate::schemas::Application> {
                self.fields = Some(concat!("nextPageToken,", "applications", "(*)").to_owned());
                crate::iter::PageItemIter::new(self, "applications")
    ...
    pub mod transfers {
        ...
        impl<'a, A: yup_oauth2::GetToken> InsertRequestBuilder<'a, A> {
            ...
            fn _request(&self, path: &str) -> ::reqwest::RequestBuilder {
                ...
                let req = req.query(&[("userIp", &self.user_ip)]);
                let mut auth = self.auth.lock().unwrap();
                let fut = auth.token(vec!["https://www.googleapis.com/auth/admin.datatransfer"]);
                let mut runtime = ::tokio::runtime::Runtime::new().unwrap();
                let token = runtime.block_on(fut).unwrap().access_token;
                let req = req.bearer_auth(&token);
                req
        ...
        impl<'a, A: yup_oauth2::GetToken> ListRequestBuilder<'a, A> {
            ...
            #[doc = r" populated fields in the yielded items will be determined by the"]
            #[doc = r" `FieldSelector` implementation."]
            pub fn iter_data_transfers<T>(mut self) -> crate::iter::PageItemIter<Self, T>
            where
                T: ::serde::de::DeserializeOwned + ::field_selector::FieldSelector,
            {
                let mut fields = concat!("nextPageToken,", "dataTransfers").to_owned();
                let items_fields = T::field_selector();
                if !items_fields.is_empty() {
                    fields.push_str("(");
                    fields.push_str(&items_fields);
            ...
            #[doc = r" fields in `#items_type` will be the default fields populated by"]
            #[doc = r" the server."]
            pub fn iter_data_transfers_standard(
                mut self,
            ) -> crate::iter::PageItemIter<Self, crate::schemas::DataTransfer> {
                self.fields = Some(concat!("nextPageToken,", "dataTransfers").to_owned());
                crate::iter::PageItemIter::new(self, "dataTransfers")
            }
            #[doc = r" Return an iterator that iterates over all `#prop_ident`. The"]
            #[doc = r" items yielded by the iterator are `#items_type`. The populated"]
            ...
            #[doc = r" all fields can be expensive both in bandwidth and server"]
            #[doc = r" resources."]
            pub fn iter_data_transfers_debug(
                mut self,
            ) -> crate::iter::PageItemIter<Self, crate::schemas::DataTransfer> {
                self.fields = Some(concat!("nextPageToken,", "dataTransfers", "(*)").to_owned());
                crate::iter::PageItemIter::new(self, "dataTransfers")
