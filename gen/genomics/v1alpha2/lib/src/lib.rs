...
pub mod resources {
    ...
    pub mod pipelines {
        ...
        impl<'a> PipelinesActions<'a> {
            ...
            #[doc = "Creates a pipeline that can be run later. Create takes a Pipeline that has all fields other than `pipelineId` populated, and then returns the same pipeline with `pipelineId` populated. This id can be used to run the pipeline. Caller must have WRITE permission to the project."]
            ...
            #[doc = "Deletes a pipeline based on ID. Caller must have WRITE permission to the project."]
            ...
            #[doc = "Retrieves a pipeline based on ID. Caller must have READ permission to the project."]
            ...
            #[doc = "Gets controller configuration information. Should only be called by VMs created by the Pipelines Service and not by end users."]
            ...
            #[doc = "Lists pipelines. Caller must have READ permission to the project."]
            ...
            #[doc = "Sets status of a given operation. Any new timestamps (as determined by description) are appended to TimestampEvents. Should only be called by VMs created by the Pipelines Service and not by end users."]
        ...
        impl<'a> GetControllerConfigRequestBuilder<'a> {
            ...
            async fn _request(
                ...
                req = req.query(&[("validationToken", &self.validation_token)]);
                req = req.query(&[("access_token", &self.access_token)]);
                req = req.query(&[("alt", &self.alt)]);
                req = req.query(&[("callback", &self.callback)]);
                req = req.query(&[("fields", &self.fields)]);
        ...
        impl<'a> ListRequestBuilder<'a> {
            ...
            async fn _request(
                ...
                let mut req = self.reqwest.request(::reqwest::Method::GET, path);
                req = req.query(&[("namePrefix", &self.name_prefix)]);
                req = req.query(&[("pageSize", &self.page_size)]);
                req = req.query(&[("pageToken", &self.page_token)]);
