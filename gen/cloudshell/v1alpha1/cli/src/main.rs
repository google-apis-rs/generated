...
impl<'a, 'b> Default for HeapApp<'a, 'b> {
    fn default() -> Self {
        ...
        {
            let mcmd = SubCommand::with_name("create").about("Adds a public SSH key to an environment, allowing clients with the corresponding private key to connect to that environment via SSH. If a key with the same format and content already exists, this will return the existing key.");
            public_keys2 = public_keys2.subcommand(mcmd);
        }
        {
            let mcmd = SubCommand::with_name("delete").about("Removes a public SSH key from an environment. Clients will no longer be able to connect to the environment using the corresponding private key.");
            public_keys2 = public_keys2.subcommand(mcmd);
        }
